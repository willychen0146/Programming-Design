...
int main(){
  ...
  // create their id and score list pointer
  int** id_list = nullptr;
  int** score_list = nullptr;

  // create other recording array pointer
  ...
  for(int i = 0; i < total_person; i++){
    ...
    // get different person movie id and score number
  ...
  // If read_score = 0, sum the total score to read_score array and record the viewing frequency to frequency_score array
  for(int i = 0; i < total_film; i++){
    if(read_score[i] == 0){
  ...
  // find the highest scored movie (compute score first, if identity --> compute frequency)
  max_score = 0;
  max_score_id = -1;
  for(int i = 0; i < total_film; i++){
